	   ...
	   //use_11=1;
		...
		if (cookie) {
			...
			while (b != NULL && b[0] != '\0' && max_cookies-- > 0) {
				...
				print_buffer(&bstr, "%s", cookie_expanded.cook_name);
				x_escape_flags(cookie_expanded.cook_value, buffer, sizeof(buffer), ENCODE_PARAMS_COOKIE);
				print_buffer(&bstr, "=%s", buffer);//slightly more than RFC required but most servers encode uri so sohuld be fine
				b = cookie_nextfield(b);

			}
			if (!first) {               // on a envoyé un (ou plusieurs) cookie?
...
HTSEXT_API size_t escape_spc_url(const char* const src,
	...
	return x_escape_flags(src, dest, size, ENCODE_PARAMS_SPACE);
}

// smith / john -> smith%20%2f%20john
HTSEXT_API size_t escape_in_url(const char* const src,
	char* const dest, const size_t size) {
	return x_escape_flags(src, dest, size, ENCODE_PARAMS_URL_ENCODING);
}

// smith / john -> smith%20/%20john
HTSEXT_API size_t escape_uri(const char* const src,
	char* const dest, const size_t size) {
	return x_escape_flags(src, dest, size, ENCODE_PARAMS_URI_COMPONENT);
}

HTSEXT_API size_t escape_uri_utf(const char* const src,
	char* const dest, const size_t size) {
	return x_escape_flags(src, dest, size, ENCODE_PARAMS_URI_COMPONENT_UTF);
}

HTSEXT_API size_t escape_check_url(const char* const src,
	char* const dest, const size_t size) {
	return x_escape_flags(src, dest, size, ENCODE_PARAMS_SPACE_QUOTE_SPECIAL);
}

/*
case 0:
		mode = ENCODE_PARAMS_SPACE_QUOTE_SPECIAL;
		break;
	case 1:
		mode = ENCODE_PARAMS_URL_ENCODING;
		break;
	case 2:
		mode = ENCODE_PARAMS_SPACE;
		break;
	case 3:
		mode = ENCODE_PARAMS_URI_COMPONENT;
		break;
	case 30:
		mode = ENCODE_PARAMS_URI_COMPONENT_UTF;
		break;
...
HTSEXT_API size_t x_escape_flags(const char* const s, char* const dest, const size_t size, ENCODE_PARAMS_t flags) {
	...
	for (i = 0, j = 0; s[i] != '\0'; i++) {
		...
		if (flags & ENCODE_PARAMS_SPACE)
			test |= c == ' ';
		if (flags & ENCODE_PARAMS_DBL_QUOTES)
			test |= c == '"';
		if (flags & ENCODE_PARAMS_SPECIAL_UTF)
			test |= CHAR_SPECIAL(c);
		if (flags & ENCODE_PARAMS_MARK)
			test |= CHAR_MARK(c);
		if (flags & ENCODE_PARAMS_DELIM)
			test |= CHAR_DELIM(c);
		if (flags & ENCODE_PARAMS_RESERVED)
			test |= CHAR_RESERVED(c);
		if (flags & ENCODE_PARAMS_COOKIE)
			test |= CHAR_COOKIE(c);
		if (flags & ENCODE_PARAMS_XXAVOID)
			test |= CHAR_XXAVOID(c);
		if (flags & ENCODE_PARAMS_UNWISE)
			test |= CHAR_UNWISE(c);
		if (flags & ENCODE_PARAMS_HIGH)
			test |= CHAR_HIG(c);
		if (flags & ENCODE_PARAMS_AMPERSAND)
			test |= c == '&';

		if (flags & ENCODE_PARAMS_NOT_SLASH)
			test &= c != '/';
		if (flags & ENCODE_PARAMS_NOT_PLUS)
			test &= c != '+';
...
HTSEXT_API size_t x_escape_http(const char* const s, char* const dest,
	const size_t size, const int _mode) {
	ENCODE_PARAMS_t mode = 0;
	switch (_mode) {
	case 0:
		mode = ENCODE_PARAMS_SPACE_QUOTE_SPECIAL;
		break;
	...
	return x_escape_flags(s, dest, size, mode);

}

HTSEXT_API size_t escape_for_html_print(const char* const s, char* const dest, const size_t size) {
	return x_escape_flags(s, dest, size, ENCODE_PARAMS_AMPERSAND | ENCODE_PARAMS_AS_HTML_ENTITIES);
}

HTSEXT_API size_t escape_for_html_print_full(const char* const s, char* const dest, const size_t size) {

	return x_escape_flags(s, dest, size, ENCODE_PARAMS_AMPERSAND | ENCODE_PARAMS_AS_HTML_ENTITIES | ENCODE_PARAMS_HIGH);
}
